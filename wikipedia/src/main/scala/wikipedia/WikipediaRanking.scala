...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((accumulator, wiki) => if (wiki.mentionsLanguage(lang)) accumulator + 1 else accumulator, _+_)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
      rdd
        .flatMap(wikiArticle => langs.map(l => (l, wikiArticle, wikiArticle.mentionsLanguage(l)))) // RDD[String, WikipediaArticle, Boolean]
        .filter(_._3)
        .map(wikiArticle => (wikiArticle._1, wikiArticle._2)) // RDD[(String, WikipediaArticle)]
