 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of positive and negative numbers`: Unit = {
    Assert.assertEquals(sum(List(1,2,-2,1,-2,-3)), -3)
  }

  @Test def `sum of emptyList`: Unit = {
    Assert.assertEquals(sum(List()), 0)
  }

  @Test def `sum of zeros`: Unit = {
    Assert.assertEquals(sum(List(0,0,0)), 0)
  }
  ...
  @Test def `max of negative numbers`: Unit = {
    Assert.assertEquals(max(List(-12, -10, -2,  -2)), -2)
  }

  @Test def `max of emptyList`: Unit = {
    try {
      max(List())
    } catch {
  ...
  @Test def `max of list of zeros`: Unit = {
    Assert.assertEquals(max(List(0,0,0)), 0)
  }
